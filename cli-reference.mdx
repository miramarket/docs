...
Full reference for every `miramarket-cli` command. For installation and a first walkthrough, see [CLI](/cli). For the strategy JSON format and correctness rules, see [Build strategies with an AI agent](/agent-strategy-guide).
...
See [Build strategies with an AI agent](/agent-strategy-guide) for the JSON format itself and the rules for building a strategy that behaves correctly, not just one that passes `validate`.
